...
issues.get("/", (req, res, next) => {
  db.all(
    `select * from Issue where series_id = ${req.series.id}`,
    (err, rows) => {
...
issues.post("/", (req, res, next) => {
  ...
  values ('${name}', '${num}', '${pub}', ${artId}, ${serId})`,
    function (err) {
      if (err) next(err);
      else
...
issues.put("/:issueId", (req, res, next) => {
  ...
  db.run(
    `update Issue set name = '${name}', issue_number = '${num}', artist_id = '${artId}',
  publication_date = '${pub}' where id = ${req.issue.id};`,
    function (err) {
      if (err) next(err);
      else
